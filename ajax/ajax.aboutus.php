...
require_once dirname(__DIR__) . '/classes/class.aboutus.php';
...
if (isset($_REQUEST['action'])) { 
    switch ($_REQUEST['action']) {
        ...
        case 'getcategorycontent':
            $cate_id = $_SESSION['fix_cateid'];
            $category = $mydata->get_category($cate_id); 
        ...
        case 'getAboutusVideo':
            ProtectedWeb::login_only();
            ProtectedWeb::methodPostOnly();

            $sql = "SELECT * FROM aboutus_video WHERE video IS NOT NULL OR video <> ''";
            $result = $dbcon->query($sql);
        ...
        case 'delAboutusVideo':
            ...
            if(is_numeric($_POST['number'])){
                ...
                $id = filter_var($_POST['number'],FILTER_SANITIZE_NUMBER_INT);
