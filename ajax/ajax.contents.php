...
if (isset($_REQUEST['action'])) {
    ...
    switch ($_REQUEST['action']) {
        ...
        case 'getcategorycontent':
            ...
            $cate_id = explode(',',$getpost['cate_id']);
            if(isset($cate_id[1])){
                foreach($cate_id as $value){
                    $category_arr[] = $category[$value];
                }
            }else{
                $category_arr[] = $category[$getpost['cate_id']];
            } 
            $category_right = $mydata->get_category_tree($category_arr);
        ...
        case 'addcontent':
            ...
            if($priority == 0 || $priority == ""){
                $sql ="SELECT max(priority) FROM post  WHERE category = 5";
                $result = $dbcon->fetch_assoc($sql);
                $priority = $result++;
            }
        ...
        case 'editcontent':
            ...
            if ($check_slug) {
                //ค่า true คือ slug สามารถเอาไปใช้งานได้
                //แก้ไขข้อมูล content
                if ($_REQUEST['submit-type'] == 'edit') {
                    ...
                    $value = array(
                        ...
                        ":meta_title" =>$_REQUEST['edit-meta-title'],
                        ":meta_description" =>$_REQUEST['edit-meta-description'],
                ...
                // เพิ่มเนื้อหาภาษาใหม่
                    ...
                    $param = ":id,:title,:keyword,:description,:slug,:topic,:freetag,:thumbnail,:h1,:h2,:video,:category,:tag,
								:content,:saleprice,:specialprice,:link_fb,:link_tw,:link_ig,:display,:date_created,
        ...
        case 'uploadimgcontent':
            ...
            $name_image = explode('.',$_FILES['images']['name'][0])[0];
            $table = "post";
            $set = "thumbnail = '" . $images['0'] . "', thumbnail2 = '".$name_image."'";
