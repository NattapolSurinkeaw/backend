...
function initEdit() {
  ...
  $("#save-edit").on("click", function () {
    ...
    $('#imgmoreId-edit').val($(".id_imgmore-edit").map(function () { return $(this).data("id"); }).get());

    if($('#edit-title').val().length < 20 || $('#edit-title').val().length > 70){
      $('#edit-title').focus();
    ...
    if($('#edit-meta-title').val().length < 30 || $('#edit-meta-title').val().length > 60){
      $('#edit-meta-title').focus();
      $(".form-edit-meta-title").addClass("has-error");
      $(".edit-meta-title-error").css("display","block");
      return false;
    } else {
      $(".form-edit-meta-title").removeClass("has-error");
      $(".edit-meta-title-error").css("display","none");
    }

    if($('#edit-description').val().length < 70 || $('#edit-description').val().length > 155){
    ...
    if ($("#form-edit-content").valid()) {
      var url = url_ajax_request + "ajax/ajax.contents.php";
      $.ajax({
        type: "POST",
        url: url,
        dataType: 'json',
        data: $("#form-edit-content").serializeArray(),
...
function load_edit() {
  ...
  $.ajax({
    ...
    success: function (obj) {
      ...
      var tag_list = '', img_list = '';
      //เลือกค่าเดิม category 
      if( $.jstree.reference('#blog-category-tree')) {
      ...
      $('#edit-meta-title').val(obj.meta_title);
      $('#edit-meta-description').val(obj.meta_description);
...
function uploadimages(id, action) {
  ...
  $.ajax({
    url: url_ajax_request + "ajax/ajax.contents.php",
    type: 'POST',
    data: formdata,
    processData: false,
    contentType: false,
