<?php
  $getpost['cateid'] = isset($_GET['bycate']) ? $_GET['bycate'] : 5;
...
<div class="content-wrapper">
  <section class="content-header">
    <h1>
      <i class="fa fa-newspaper-o"></i> <?php echo $LANG_LABEL['content'];?> 
        ...
        <div class="box box-primary">
          ...
          <div class="categorybox-controls">
            ...
            ?>
              ...
              <select class="form-control"  id="cate-id" style="width: 200px; display: inline-block; margin: 0 10px 5px 5px;">
                <option value=""><?php echo $LANG_LABEL['allcate'];?></option>
              <?php
              $search_bycate = isset($_GET['bycate']) ? $_GET['bycate'] : "";
              foreach ($category as $value) {
                  if ($value['main_page'] != 'yes') {
                    if($search_bycate == $value['cate_id']) {
                      $select = 'SELECTED';
                    }else {
                      $select = '';
                    }
                    if($value['cate_id'] ==5){
                      echo '<option value="'.$value['cate_id'].'" '.$select.'>'.$value['cate_name'].'</option>';
                    } 
                  }
                }
              ?>
            ...
            <div class="pull-right">
              <button type="button" class="btn btn-primary bt-add-content kt:btn-info" data-toggle="modal"><i class="fa fa-plus"></i> <?php echo $LANG_LABEL['addcontent'];?></button>
            </div>
            <?php
              }else {
            ?>
              ...
              <div class="attachment-block clearfix" <?php if ($post['display'] == 'no') {echo 'style="opacity: 0.6;"';} ?> >
                ...
                <div class="content-info">
                  <h1>
                    ...
                    <label class = "showclass" style="width:unset;">id : <?php echo $post['id']; ?></label>
                    <?php } ?>
                  </h1>
                     ...
                     <!-- คัดกรองให้ลบได้เฉพาะ SUPERADMIN || ADMIN --> 
                      ...
                      if($_SESSION['role'] != "superadmin" && $_SESSION['role'] != "admin"){
                          if($post['section'] != "hide"){ //user and editor zone
                              if($post['allow_delete'] == "yes"){ 
                              ?> 
                              <button type="button" class="btn btn-danger delete-content" data-id="<?php echo $post['id']; ?>">
...
<?php
  include 'template/contents/addcontent.php';
  include 'template/contents/editcontent.php';
?>
<!-- css -->
...
<script src="<?php echo SITE_URL; ?>js/jquery/jquery-validate1-19-0.js"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contents/contents.js"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contents/addcontent.js"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contents/editcontent.js?v=123<?=date('dhis')?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contents/hidebtn.js"></script>
