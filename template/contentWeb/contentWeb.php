<?php
	...
	$getpost['subcate'] = $_REQUEST['subcate']; 
  $getpost['status'] = $_REQUEST['status'];
  $getpost['cate_slc'] = $_REQUEST['cate'];
  $getpost['sortby'] = "de";
	$all_posts = $mydata->get_post($getpost);
  $category = $mydata->get_category();
   
	$getpost['bycate']  = $_REQUEST['bycate'];
	$subcategory = $mydata->get_sub_category($getpost); 
?>
<div class="content-wrapper">
  <section class="content-header">
    <h1>
      <i class="fa fa-newspaper-o"></i> เนื้อหาเว็บไซต์ 
    
    </h1>
    <ol class="breadcrumb">
      <li><a href="<?php echo SITE_URL; ?>"><i class="fa fa-dashboard"></i> หน้าหลัก</a></li>
      <li class="active">เนื้อหาเว็บไซต์</li>
    </ol>
  </section>
        ...
        <div class="box box-primary">
          ...
          <div class="categorybox-controls">
            ...
            ?>
              ...
              <select class="form-control" id="categorySlc" style="width: 140px; display: inline-block; margin: 0 10px 5px 5px;">
                <option value="all">ทั้งหมด</option> 
                <option value="nav" <?php if($_REQUEST['cate']=='nav'){echo 'SELECTED';} ?>>เมนูด้านข้าง</option>  
                <option value="cart" <?php if($_REQUEST['cate']=='cart'){echo 'SELECTED';} ?>>ตะกร้าสินค้า</option>  
                <option value="howto" <?php if($_REQUEST['cate']=='howto'){echo 'SELECTED';} ?>>หน้าวิธีสั่งซื้อ</option>  
                <option value="about" <?php if($_REQUEST['cate']=='about'){echo 'SELECTED';} ?>>หน้าเกี่ยวกับเรา</option>  
                <option value="contact" <?php if($_REQUEST['cate']=='contact'){echo 'SELECTED';} ?>>หน้าติดต่อเรา</option>   
                <option value="seo" <?php if($_REQUEST['cate']=='seo'){echo 'SELECTED';} ?>>SEO</option>   
              ...
              <select class="form-control" id="sort-by" style="width: 110px; display: inline-block; margin: 0 10px 5px 5px;">
                ...
                <option value="df" <?php if($_REQUEST['sortby']=='df'){echo 'SELECTED';} ?>>ลำดับการแสดง</option>
              </select> -->
            ...
            <div class="pull-right">
              <!-- <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalAddContent"><i class="fa fa-plus"></i> เพิ่มบทความ</button> -->
            </div>
            <?php
              }else {
            ?>
              ...
              <div class="attachment-block clearfix" <?php if ($a['display'] == 'no') {echo 'style="opacity: 0.6;"';} ?> >
                ...
                <div class="content-info">
                  <h1>
                    ...
                    <?php echo $a['title']." <span style='color:mediumseagreen'>[id:".$a['id']."]</span>"; ?>
                    <?php
                      if ($a['display'] == 'no') {
                        echo '
                    ...
                    ?>
                    <?php 
                        if ($_SESSION['role'] === 'superadmin' && $_SESSION['login_user_id'] === '725' || $_SESSION['login_user_id'] === '167'|| $_SESSION['login_user_id'] === '730' || $_SESSION['login_user_id'] === '731') {
                    ?>
                    <label class = "showclass">id : <?php echo $a['id']; ?></label>
                    <?php } ?>
                  </h1>

                  <p class="text-category"><i class="fa fa-folder-open-o" aria-hidden="true"></i> 
                    <?php $catename = $mydata->getcontentcate($a['category']); ?>
                    <?php echo substr( $catename['catename'] ,0,-2); ?>
                  ...
                  <p>ลำดับการแสดง: <strong style="color:red"><?=$a['priority']?></strong></p>
                ...
                <div class="content-button pull-right">
                  ...
                  ?>
                    ...
                    <button type="button" class="btn btn-primary margin-r-10 edit-content" data-id="<?php echo $a['id']; ?>" data-type="add" >
...
<?php
  include 'template/contentWeb/addContentWeb.php';
  include 'template/contentWeb/editContentWeb.php';
?>
<!-- css -->
...
<script src="<?php echo SITE_URL; ?>js/pages/contentWeb/contents.js?v=<?=date('mdhis')?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contentWeb/addcontent.js?v=<?=time()?>"></script>
<script src="<?php echo SITE_URL; ?>js/pages/contentWeb/editcontent.js?v=<?=time()?>"></script>
