...
class transmit
{
    ...
    {
        $sql = "SELECT  cate_id as 'id',category.* FROM category WHERE cate_id IN (".$catid.") ORDER BY level, priority ASC, position ASC, FIELD( defaults,  'yes')DESC";
    ...
    public function get_post($getpost)
    {
        ...
        $sort_src = (isset($cate) && $cate != '' && $cate == 24)? $sort_src = 'example DESC,': ""; 
        ...
        if ($sort == "priority") {
            $sql = "SELECT * FROM post INNER JOIN (SELECT id FROM post WHERE title LIKE '%" . $search . "%' " . $cate_src . " " . $status . " GROUP BY id ORDER BY " . $sort . " DESC LIMIT " . $lim . ")p ON p.id = post.id ORDER BY ".$sort_src . $sort . " ASC, p.id DESC, FIELD( defaults,'yes')DESC";
        } else {
            $sql = "SELECT * FROM post INNER JOIN (SELECT id FROM post WHERE title LIKE '%" . $search . "%' " . $cate_src . " " . $status . " GROUP BY id ORDER BY " . $sort . " DESC LIMIT " . $lim . ")p ON p.id = post.id ORDER BY ".$sort_src . $sort . " DESC, p.id DESC, FIELD( defaults,'yes')DESC";
        }
