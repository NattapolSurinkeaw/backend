...
class chart_order
{
    ...
    {
        $sql = "SELECT  cate_id as 'id',category.* FROM category ORDER BY level, priority ASC, position ASC, FIELD( defaults,  'yes')DESC";
	...
	*/
    ...
    public function getMemberIdNameGeneral(){
        $sql = "SELECT member_id,member_name FROM members WHERE member_type = 'general' OR 'special' ";
        $res = $this->dbcon->fetchAll($sql,[]);
        return $res;
    }
    public function getMemberIdNameHospital(){
        $sql = "SELECT member_id,member_name FROM members WHERE member_type = 'hospital'";
        $res = $this->dbcon->fetchAll($sql,[]);
        return $res;
    }

    public function getYearFromOrderGeneral(){
        $sql = "SELECT MIN(YEAR(create_date))as min FROM order_general";
        $res = $this->dbcon->fetchObject($sql,[]);
        return $res->min;
    }
    public function getYearFromOrderHospital(){
        $sql = "SELECT MIN(YEAR(create_date))as min FROM order_hospital";
        $res = $this->dbcon->fetchObject($sql,[]);
        return $res->min;
    }

    public function getYearOrderMin(){
        $sql = "SELECT MIN(YEAR(create_date))as min FROM order_general";
        $resG = $this->dbcon->fetchObject($sql,[]);
        // $resG->min;

        $sql = "SELECT MIN(YEAR(create_date))as min FROM order_hospital";
        $resH = $this->dbcon->fetchObject($sql,[]);
        // $resH->min;

        if($resG->min < $resH->min){
            return $resG->min;
            // echo "resG";
        }else if($resG->min > $resH->min){
            return $resH->min;
            // echo "resH";
        }else {
            return $resG->min;
    ...
    public function getProductAll(){
        $sql = "SELECT id,title FROM post WHERE category = 3";
        $resProductAll = $this->dbcon->fetchAll($sql,[]);
        return $resProductAll;
    }
}
