...
class car_cate {
	...
	public function get_roomcategory(){
		$lan_arr = $this->lan_arr;
		$sql = "SELECT * FROM car_cate ORDER BY level, priority ASC, position ASC, FIELD( defaults,  'yes')DESC";
	...
	public function get_all_roomcategory($getpost, $status){
		$pagi  = $getpost['pagi'] ;
		...
		if (!empty($search)) {
			$sql = "SELECT * FROM car_cate INNER JOIN (SELECT cate_id FROM car_cate WHERE cate_name LIKE '%".$search."%' GROUP BY cate_id)p ON p.cate_id = car_cate.cate_id ORDER BY parent_id, level, priority ASC, position ASC, FIELD( defaults,  'yes')DESC";
		}else {
			$sql = "SELECT * FROM car_cate INNER JOIN (SELECT cate_id FROM car_cate GROUP BY cate_id LIMIT ".$lim.")p ON p.cate_id = car_cate.cate_id ORDER BY parent_id, level, priority ASC, position ASC, FIELD( defaults,  'yes')DESC";
		}
