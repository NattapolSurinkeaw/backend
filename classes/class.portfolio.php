...
class portfolio
{
    ...
    public function get_post($getpost)
    {
        ...
        if (isset($getpost['pin']) && $getpost['pin'] != '') {
            $portfolio_pin = " AND pin = '" . $getpost['pin'] . "'";
        }
        ...
        if ($sort == "priority") {
            $sql = "SELECT * FROM post INNER JOIN (SELECT id FROM post WHERE title LIKE '%" . $search . "%' " . $cate_src . " " . $status . " ".$portfolio_pin." GROUP BY id ORDER BY " . $sort . " DESC LIMIT " . $lim . ")p ON p.id = post.id ORDER BY " . $sort . " ASC, p.id DESC, FIELD( defaults,'yes')DESC";
        } else {
            $sql = "SELECT * FROM post INNER JOIN (SELECT id FROM post WHERE title LIKE '%" . $search . "%' " . $cate_src . " " . $status . " ".$portfolio_pin." GROUP BY id ORDER BY " . $sort . " DESC LIMIT " . $lim . ")p ON p.id = post.id ORDER BY " . $sort . " DESC, p.id DESC, FIELD( defaults,'yes')DESC";
        }
